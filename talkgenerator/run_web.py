...
def talkgen_generate():
    ...
    if "slides" not in request.args or request.args["slides"] in ("", None):
        return notify_error(
            "ERR_NO_ARG:  'slides' argument required", HTTP_ERROR_CLIENT
        )
    ...
    if "slides" in request.args and request.args["slides"] not in ("0", None):
        try:
            ...
            return notify_error(
                "ERR_NO_ARG:  'slides' must be an integer", HTTP_ERROR_CLIENT
            )

        if slides < 1 or slides > 15:
            return notify_error(
                "ERR_OUT_OF_BOUNDS:  'slides' parameger must be between 1 and 15",
                HTTP_ERROR_CLIENT,
            )
...
def gather_run_params(topic, slides):
    ...
    return ["--topic", topic, "--num_slides", num_slides, "--open_ppt", "false"]
